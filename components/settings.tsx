...
const getNarrationLanguageLabel = (narrationLanguage: Language, currentAppLanguage: Language): string => {
    const translationKey = `settings.language.${narrationLanguage}`;
    return translate(currentAppLanguage, translationKey);
}
...
export function Settings({
    ...
    const userLanguage = navigator.language || "en-UK";
    ...
    const filteredVoices = voices.filter((voice) => {
        ...
        return lang === supportedLanguagesConfig.en.value || lang === supportedLanguagesConfig.pl.value || lang === supportedLanguagesConfig.de.value
    })

    // Group voices by language for display purposes only
    const englishVoices = filteredVoices.filter((voice) => voice.lang.startsWith(supportedLanguagesConfig.en.value))
    const polishVoices = filteredVoices.filter((voice) => voice.lang.startsWith(supportedLanguagesConfig.pl.value))
    const germanVoices = filteredVoices.filter((voice) => voice.lang.startsWith(supportedLanguagesConfig.de.value))

    const languageLabel = getNarrationLanguageLabel(narrationLanguage, language)
        ...
        <div className="space-y-6 p-4">
            ...
            <div className="flex items-center justify-between">
                <Label htmlFor="narration-toggle" className="flex flex-col space-y-1">
                    ...
                    <span
                        className="font-normal text-sm text-muted-foreground">{t("settings.narration.description")}</span>
                </Label>
